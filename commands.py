...
class ProjectProfileMutationCommand(QUndoCommand):
    def __init__(self, model, project_task_id: int, text: str, apply_fn):
        super().__init__(text)
        self.model = model
        self.project_task_id = int(project_task_id)
        self.apply_fn = apply_fn
        self.before = deepcopy(model.capture_project_profile_snapshot(self.project_task_id))
        self.after = None
    ...
    def redo(self):
        if self.after is None:
            self.apply_fn()
            self.after = deepcopy(
                self.model.capture_project_profile_snapshot(self.project_task_id)
            )
            if self.after == self.before:
                self.setObsolete(True)
                return
        else:
            self.model._restore_project_profile_snapshot(self.after)
        self.model.reload_all(reset_header_state=False)

    def undo(self):
        if self.before is None:
            return
        self.model._restore_project_profile_snapshot(self.before)
        self.model.reload_all(reset_header_state=False)
